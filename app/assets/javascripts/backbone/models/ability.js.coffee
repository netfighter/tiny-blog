(->
  (->
    root = undefined
    root = this
    root.Ability = Backbone.Model.extend(
      defaults: ->
        {
          rules: []
          aliased_actions:
            read: [
              'index'
              'show'
            ]
            create: [ 'new' ]
            update: [ 'edit' ]
        }
      initialize: ->
        if !_.isEmpty(@get('rules'))
          @set 'rules', _.map(@get('rules'), (rule) ->
            new Rule(rule)
          )
        return
      can: (action, subject) ->
        match = undefined
        match = _.detect(@relevant_rules(action, subject), ((rule) ->
          rule.matches_conditions action, subject
        ), this)
        if match
          match.get 'base_behavior'
        else
          false
      cannot: (action, subject) ->
        !@can(action, subject)
      set_can: (action, subject, conditions) ->
        @get('rules').push new Rule(
          base_behavior: true
          action: action
          subject: subject
          conditions: conditions)
        return
      set_cannot: (action, subject, conditions) ->
        @get('rules').push new Rule(
          base_behavior: false
          action: action
          subject: subject
          conditions: conditions)
        return
      alias_action: (from, target) ->
        @validate_target target
        if !_.isArray(@get('aliased_actions')[target])
          @get('aliased_actions')[target] = []
        @get('aliased_actions')[target] = @get('aliased_actions')[target].concat(from)
        return
      validate_target: (target) ->
        if _.chain(@get('aliased_actions')).values().flatten().include(target).value()
          throw new Error('You can\'t specify target (' + target + ') as alias because it is real action name')
        return
      clear_aliased_actions: ->
        @set 'aliased_actions', {}
        return
      expand_actions: (actions) ->
        _.chain(actions).map(((action) ->
          if @get('aliased_actions')[action]
            [ action ].concat @expand_actions(@get('aliased_actions')[action])
          else
            action
        ), this).flatten().value()
      relevant_rules: (action, subject) ->
        reversed_rules = undefined
        reversed_rules = @get('rules').slice(0)
        _.select reversed_rules.reverse(), ((rule) ->
          rule.set 'expanded_actions', @expand_actions(rule.get('actions'))
          rule.is_relevant action, subject
        ), this
    )
    root.Rule = Backbone.Model.extend(
      initialize: ->
        if !@get('actions') and @get('action')
          @set 'actions', _.flatten([ @get('action') ])
        if !@get('subjects') and @get('subject')
          @set 'subjects', _.flatten([ @get('subject') ])
        if !@get('conditions')
          @set 'conditions', {}
        return
      backbone_class: (sub) ->
        if sub == null
          sub = null
        if sub != null then sub.backboneClass else undefined
      is_relevant: (action, subject) ->
        @matches_action(action) and @matches_subject(subject)
      matches_conditions: (action, subject) ->
        if _.isObject(@get('conditions')) and !_.isEmpty(@get('conditions')) and !@subject_class(subject)
          @matches_conditions_hash subject
        else
          if _.isEmpty(@get('conditions'))
            true
          else
            @get 'base_behavior'
      subject_class: (subject) ->
        if @backbone_class(subject)
          true
        else
          false
      matches_action: (action) ->
        _.include(@get('expanded_actions'), 'manage') or _.include(@get('expanded_actions'), action)
      matches_subject: (subject) ->
        _.include(@get('subjects'), 'all') or _.include(@get('subjects'), subject) or @matches_subject_class(subject)
      matches_subject_class: (subject) ->
        _.any @get('subjects'), ((_this) ->
          (sub) ->
            sub_class = undefined
            subject_class = undefined
            sub_class = _this.backbone_class(sub) or _this.backbone_class(sub.constructor)
            subject_class = _this.backbone_class(subject) or _this.backbone_class(subject.constructor)
            sub_class and subject_class and sub_class == subject_class or sub == subject_class or sub_class == subject
        )(this)
      matches_conditions_hash: (subject, conditions) ->
        if !conditions
          conditions = @get('conditions')
        if _.isEmpty(conditions)
          true
        else
          _.all conditions, ((value, name) ->
            attribute = undefined
            attribute = subject[name]
            if _.isUndefined(attribute)
              attribute = subject.get(name)
            if _.isObject(value) and !_.isArray(value)
              if _.isArray(attribute)
                _.any attribute, ((element) ->
                  @matches_conditions_hash element, value
                  return
                ), this
              else
                attribute and @matches_conditions_hash(attribute, value)
            else if _.isArray(value)
              _.include(value, attribute) or _.isEqual(value, attribute)
            else
              attribute == value
          ), this
    )
    return
  ).call this
  return
).call this
